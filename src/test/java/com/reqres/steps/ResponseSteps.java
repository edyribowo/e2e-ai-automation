...
public class ResponseSteps {
    ...
    private Response response() { return context.getResponse(); }
    private JsonPath json()     { return response().jsonPath(); }
    ...
    public void theResponseFieldShouldEqualString(String field, String expected) {
        assertThat("field '" + field + "'", json().getString(field), is(equalTo(expected)));
    }
    ...
    public void theTimestampShouldBeWithinSecondsOfNow(String field, int seconds) {
        ...
        long delta = Math.abs(Duration.between(timestamp, Instant.now()).getSeconds());
        assertThat("'" + field + "' should be recent", delta, is(lessThan((long) seconds)));
    }
